...
# Create synchronization class for buttons
class Buttonn():
...
# Create layout for buttons on keyboard
def transparent_layout(img, buttonList):
    imgNew = np.zeros_like(img, np.uint8)
    # Create new frame on img
    ...
    # alpha = 0.5 : Trong so muc sang anh 1
    # beta = 1 - alpha : Trong so muc sang anh 2
    # gamma = 0 : He so chinh sang pha tron
    # > Blending: src1 * alpha + src2 * beta + gamma <
    return out
...
def main_program():
    ...
    # Simulation keyboard press by Controller() from Pynput
    keyboard = Controller()
    ...
    for k in range(len(keys)):
        for x, key in enumerate(keys[k]):
            buttonList.append(Buttonn([100 * x + 25, 100 * k + 50], key))
                            # Button(          position,            text,            size[85x85])
    while True:
        ...
        # Create keyboard with layout and text on button
            ...
            for button in buttonList:
                ...
                if x < lmList[8][0] < x + w and y < lmList[8][1] < y + h:
                    ...
                    if l < 25:
                        ...
                        # Simulation button press and export notepad
